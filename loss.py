...
class Loss(nn.Module):
    def __init__(self, batch_size, class_num, device):
        ...
        self.device = device
        self.eLoss = nn.CrossEntropyLoss()
        self.mse1 = nn.MSELoss()
        self.mse2 = nn.MSELoss()

    def forward_cluster(self, hidden, output, fused=None, print_sign=False):
        ...
        if fused != None:
            l4 = 0.01*(self.mse1(fused[1], fused[0])+self.mse2(fused[1], fused[2]))
